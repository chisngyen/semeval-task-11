"""
================================================================================
SemEval-2026 Task 11 - Subtask 1: Experiment 21 - Exp17 FIXED (Valid->Invalid Only)
================================================================================
Description: Fixes the logic flaw in Exp17/19/20.
             
             Bug in Exp17: Swapping an Invalid conclusion with a Random Valid conclusion
             does NOT make the syllogism Valid. It creates False Positives.
             original: "A->B, B->C, Conc: A->not C" (Invalid)
             swap: "Conc: A->C" (Valid string) -> New Syllogism might still be Invalid!
             
             Fix: ONLY perform Valid -> Invalid augmentation.
             original: "A->B, B->C, Conc: A->C" (Valid)
             swap: "Conc: A->not C" (Invalid string) -> New Syllogism is Invalid.
             
             This removes noise from the training data.
             
Goal: Improve Accuracy/Combined Score by removing noisy labels.
...
class Config:
    ...
    OUTPUT_DIR = '/kaggle/working/deberta_exp21'
    ...
    CF_RATIO = 0.5  # Increased to 50% since we only augment Valid samples now
...
# ============================================================================
# 4. COUNTERFACTUAL GENERATOR (FIXED)
# ============================================================================
class CounterfactualGenerator:
    """
    Generate counterfactual syllogisms by swapping conclusions.
    FIXED: Only generates Valid -> Invalid pairs to avoid False Positives.
    """
    ...
    def __init__(self, all_data):
        ...
        print(f"   CF pool: {len(self.invalid_conclusions)} invalid conclusions for swapping")
    ...
    def create_counterfactual(self, item):
        """
        Create counterfactual by swapping conclusion.
        ONLY supports Valid -> Invalid transformation.
        """
        if not item['validity']:
            return None  # Skip Invalid -> Valid (unsafe without solver)
            
        # Valid -> Invalid: use random invalid conclusion from pool
    ...
    def __init__(self, data, tokenizer, max_length, is_test=False, cf_generator=None, cf_ratio=0.5):
        ...
        if not is_test and cf_generator:
            ...
            for item in data:
                ...
                if item['validity']: valid_count += 1
                else: invalid_count += 1
                
                # Counterfactual (Only for Valid items now)
                if item['validity'] and random.random() < cf_ratio:
                    cf = cf_generator.create_counterfactual(item)
                    if cf:
                        self.samples.append(cf)
                        invalid_count += 1 # CFs are always Invalid in this approach
            
            print(f"   Dataset: {len(self.samples)} samples")
            print(f"   Balance: {valid_count} Valid, {invalid_count} Invalid")
        else:
            self.samples = data
...
def main():
    print("=" * 70)
    print("Exp 21: Exp17 FIXED (Valid -> Invalid Only)")
    print("  - Removes noise from Invalid->Valid transformation")
    print("  - CF Ratio increased to 50% for Valid samples")
    print("=" * 70)
    ...
    # Validation set detailed eval
    print("\n🔍 Evaluating bias on Validation Set...")
    model.eval()
    val_preds = []
    val_labels = []
    ...
    # Calculate simple TCE proxy on validation
