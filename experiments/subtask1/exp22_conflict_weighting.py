"""
================================================================================
SemEval-2026 Task 11 - Subtask 1: Experiment 22 - Exp21 + Conflict Reweighting
================================================================================
Description: Combines two powerful techniques:
             1. Exp21's Fixed Counterfactual Augmentation (Valid->Invalid only)
                - Increases dataset size
                - Teaches structure via minimal pairs
             2. Knowledge-Conflict Reweighting (New)
                - Identifies "Conflict" samples (Validity != Plausibility)
                - Upweights their loss (3x) during training
                - Forces model to unlearn belief bias
             
Goal: Maximize Accuracy while minimizing TCE via targeted reweighting.
...
class Config:
    ...
    OUTPUT_DIR = '/kaggle/working/deberta_exp22'
    ...
    # New Hyperparameters
    CF_RATIO = 0.5         # From Exp21
    CONFLICT_WEIGHT = 3.0  # Weight for samples where Validity != Plausibility
...
# ============================================================================
# 4. COUNTERFACTUAL GENERATOR (From Exp21 - Fixed)
# ============================================================================
class CounterfactualGenerator:
    ...
    def create_counterfactual(self, item):
        ...
        if self.invalid_conclusions:
            ...
            # Counterfactuals created this way are assumed:
            # Validity: False
            # Plausibility: Generally Implausible/Unknown (we treat as Congruent or separate)
            return {
                'syllogism': new_syllogism,
                'validity': False,
                'plausibility': False, # Assign default/dummy
...
def parse_syllogism(text):
    ...
    if premise1_match and premise2_match and conclusion_match:
        return {'premise1': premise1_match.group(1), 'premise2': premise2_match.group(1), 'conclusion': conclusion_match.group(1)}
    sentences = re.split(r'[.!?]+', text)
    sentences = [s.strip() for s in sentences if s.strip()]
    if len(sentences) >= 3:
...
class ReweightedDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if not self.is_test:
            ...
            # Compute Weight: if Validity != Plausibility, weight = 3.0
            # Note: For counterfactuals, 'plausibility' might be missing or dummy.
            # We treat synthetic CFs as weight 1.0 (standard).
            weight = 1.0
            if 'plausibility' in item and 'is_counterfactual' not in item:
                if item['validity'] != item['plausibility']:
                    weight = Config.CONFLICT_WEIGHT
...
def train_epoch(model, dataloader, optimizer, scheduler, device):
    model.train()
    total_loss = 0
    ce_fn = nn.CrossEntropyLoss(label_smoothing=Config.LABEL_SMOOTHING, reduction='none') # reduction='none' for reweighting
    ...
    for batch in tqdm(dataloader, desc="Training"):
        ...
        losses = ce_fn(outputs.logits, labels)
...
def main():
    print("=" * 70)
    print("Exp 22: Exp21 (Fixed CF) + Conflict Reweighting")
    print("  - Fixed CF Augmentation (Valid->Invalid only)")
    print(f"  - Conflict Reweighting (Weight={Config.CONFLICT_WEIGHT} for Val!=Plaus)")
    print("=" * 70)
    ...
    train_dataset = ReweightedDataset(train_split, tokenizer, Config.MAX_LENGTH, 
                                      cf_generator=cf_generator, cf_ratio=Config.CF_RATIO)
    val_dataset = ReweightedDataset(val_split, tokenizer, Config.MAX_LENGTH)
    test_dataset = ReweightedDataset(test_data, tokenizer, Config.MAX_LENGTH, is_test=True)
