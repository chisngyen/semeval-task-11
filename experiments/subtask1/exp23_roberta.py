"""
================================================================================
SemEval-2026 Task 11 - Subtask 1: Experiment 23 - RoBERTa-Large + Exp21 Method
================================================================================
Description: Applying the successful "Fixed Counterfactual Augmentation" (Exp21)
             method to a different backbone: RoBERTa-Large.
             
             RoBERTa is known for robust performance on NLI/Reasoning tasks.
             We test if it generalizes better than DeBERTa-v3 on this specific task
             when debiased.
             
Model: FacebookAI/roberta-large
Method: Valid->Invalid Augmentation (Ratio 0.5)
...
class Config:
    ...
    OUTPUT_DIR = '/kaggle/working/roberta_exp23'
    ...
    BATCH_SIZE = 12
    LEARNING_RATE = 1e-5 # RoBERTa often likes slightly higher LR than DeBERTa
    WEIGHT_DECAY = 0.01
    NUM_EPOCHS = 10
    WARMUP_RATIO = 0.1
    LABEL_SMOOTHING = 0.1
    
    # Counterfactual augmentation (Exp21 setting)
    CF_RATIO = 0.5 
...
def main():
    print("=" * 70)
    print("Exp 23: RoBERTa-Large + Exp21 Method")
    print("=" * 70)
    ...
    train_dataset = CounterfactualDataset(train_split, tokenizer, Config.MAX_LENGTH, cf_generator=cf_generator, cf_ratio=Config.CF_RATIO)
    ...
    model = AutoModelForSequenceClassification.from_pretrained(os.path.join(Config.OUTPUT_DIR, 'best_model')).to(device)
